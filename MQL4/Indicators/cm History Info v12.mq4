...
int start()                                  
{
   ...
   if (SendMailInfo && AB/3>AFM && iTime(NULL,TF_SendMail,0)!=TimeSendMail_AFM) 
   {
      SendMail(StringConcatenate(NameInd," ",AccountCompany()," ",AN),
               StringConcatenate("There is not enough available assets, fill up balance of the account ",AN,"\n\nProfit ",TimeToStr(iTime(NULL,1440,0),TIME_DATE)," = ",DoubleToStr(Today[0][0],2),
   ...
   if (Win==0)
   {
      ...
      Text("DateInfo",StringConcatenate("From ",TimeToStr(DateInfoStart,TIME_DATE)," to ",TimeToStr(DateInfoEnd,TIME_DATE)),500,2,WhiteColor,8);//шапка
      string txt;
      if (MagikInfo==0) txt = "Magic Number - All";
      else  txt = StringConcatenate("Magic - ",MagikInfo);
      ...
      Text("0","Account  "+AN+" / "+AccountCompany()+" / 1:"+AccountLeverage(),100,2,WhiteColor,8);//шапка
      ...
      Text("History","Closed",120,20,SteelBlue,10);
      Text("Текушие","Open",120,93,SteelBlue,10);
   ...
   if (Ords>tn || Ords==0)
   {
      ...
      Text("symbolAll","Total",                 StartShift+Shift*i,20,SteelBlue,10);//шапка
      Text("profitAll",DoubleToStr(ProfitAll,2),StartShift+Shift*i,35,Color(ProfitAll),10);//общий профит
      i+=2;
      double AccBal=AB;
      Text("Yesterday 0","Daily Profit",StartShift+Shift*i+110,20,WhiteColor,10);
      Text("YesterdayR 0","Withdrawel",StartShift+Shift*i+270,20,WhiteColor,10);
      Text("YesterdayM 0","Deposit",StartShift+Shift*i+390,20,WhiteColor,10);
      for (j=0; j<=15; j++)
      { 
         AccBal-=Today[0][j];
...
int HistoryOrders()                                  
{
   ...
   if (TS>=0)
   {
      if (TS>0) TimeSkan = StringConcatenate(TS," ");
      if (TS==0) TimeSkan = "1 день";
   }
   
   if (EndInfo!=0) ObjectSetText("DateInfo", StringConcatenate("From ",TimeToStr(StartInfo,TIME_DATE)," To ",TimeToStr(EndInfo,TIME_DATE),"   Days ",TimeSkan), 8, "Tahoma", WhiteColor);
   else ObjectSetText("DateInfo",StringConcatenate("За данный период с ",TimeToStr(DateInfoStart,TIME_DATE)," по ",TimeToStr(DateInfoEnd,TIME_DATE)," "), 8, "Tahoma", Red);
   return(n);
}
//--------------------------------------------------------------------
int TekOrders()                                  
